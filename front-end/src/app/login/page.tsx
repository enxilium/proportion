      ...
      <div className="max-w-md w-full space-y-8 p-8 bg-[#4d2e1f] rounded-lg shadow-xl transition-all duration-300 hover:shadow-2xl hover:shadow-amber-500/30">
        <div className="text-center">
          <h2 className="mt-6 text-4xl font-bold text-white transition-colors duration-300 font-['Caveat']">
            {session ? `Welcome, ${session.user.name}!` : 'Welcome to Proportion'}
          </h2>
          <p className="mt-2 text-sm text-white/80 transition-colors duration-300">
            {session ? 'You are logged in' : 'Please sign in to your account or create a new one'}
          </p>
        </div>
        ...
        <div className="mt-8 space-y-4">
          {!session ? (
            ...
            <a href="/auth/logout" className="w-full block transition-transform duration-200 hover:scale-[1.02]">
              <button className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-amber-700 hover:bg-amber-800 transition-colors duration-300">
                Log out
              </button>
            </a>
          )}
